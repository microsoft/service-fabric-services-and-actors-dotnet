...
- **Omit nested test classes for SUT members without observable behavior**.
  ...
  - _Do test constructors initializing fields and auto-properties_. Unlike `ToString()` in the example above, which has
    to be hand-coded and therefore tested separately, a read-only field or auto-property generated by compiler doesn't
    have hand-coded implementation to be tested, so the constructor test is required.
