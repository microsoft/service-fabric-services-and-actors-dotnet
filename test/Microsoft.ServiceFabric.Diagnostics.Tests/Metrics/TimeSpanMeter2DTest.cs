...
namespace Microsoft.ServiceFabric.Diagnostics.Tests.Metrics
{
    public class TimeSpanMeterTest2D
    {
        ...
        public TimeSpanMeterTest2D() => sut = new TimeSpanMeter2D(fabricMeter, systemDimensions);

        public class Class : TimeSpanMeterTest2D
        {
            [Fact]
        ...
        public class Record : TimeSpanMeterTest2D
        {
            ...
            public void CallsFabricMeterRecordWithMultipleSystemDimensions()
            {
                ...
                // Assert
                Mock.Get(fabricMeter).Verify(m => m.Record((long)Math.Round(value.TotalMilliseconds), (uint)expectedArray.Length, It.Is<string[]>(arr => arr.SequenceEqual(expectedArray))), Times.Once);
            }
            ...
            {
                // Arrange
                string[] expectedArray = new string[] { customDimension1, customDimension2 };

                // Act
                var meter = new TimeSpanMeter2D(fabricMeter, new List<string>());
