...
namespace Microsoft.ServiceFabric.Services.Remoting.V2.Wcf.Runtime.Tests
{
    public abstract class WcfServiceRemotingListenerTest
    {
        public class Constructor
        {
            ...
            [Fact]
            public void UsesDefaultConvertorsWhenNotPassedAny()
            {
                // Arrange
                var mockEndpointsCollection = Mock.Of<KeyedCollection<string, EndpointResourceDescription>>();
                var mockCodePackageActivationContext = Mock.Of<ICodePackageActivationContext>();
                ...
                var serviceContext = mockServiceContext.Object;
                ...
                Assert.IsType<SystemExceptionConvertor>(convertorList[0]); // default
                Assert.IsType<FabricExceptionConvertor>(convertorList[1]); // default
                Assert.IsType<DefaultExceptionConvertor>(convertorList[2]); // default
            }
        }
    }
}
